...
export default config => {
	let format = Array.isArray(config.indicators) && config.indicators.length > 0
		? detectFormat(config.indicators[0].text)
		: null;
	return (
		<cx>
			<Legend.Scope>
				<Legend />
				<Svg style={`width:100%; height:600px`} controller={Controller}>
					<Chart
						margin="10 15 150 50"
						axes={{
							x: {
						...
						<Repeater records:bind="$sectionData.indicators">
							...
							>
								<Column
									...
									tooltip={{
										title: {
											tpl: `{$value.country.value}`
										},
										text: {
											tpl: `{$value.value:${format}}`
										},
										placement: "up"
									}}
								/>

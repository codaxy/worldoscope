...
export default class extends Controller {
    ...
    load() {
        ...
        if (indicatorId && fromYear && toYear && countries && countries.length > 0) {

            this.store.set('$sectionData.loading', true);

            let promises = countries
                .map(c => queryCountryIndicators(c.id, indicatorId, {
            ...
            Promise.all(promises)
                .then(countryResults => {
                    ...
                    this.store.set('$sectionData.countries', Object.keys(countryData).map(k => countryData[k]));

...
export default class extends Controller {
    ...
    load() {
        ...
        if (Array.isArray(indicators) && fromYear && toYear && country && indicators.length > 0) {

            this.store.set('$sectionData.loading', true);

            let promises = indicators
                .map(ind => {
                    return queryCountryIndicators(country.id, ind.id, {date: `${fromYear}:${toYear}`})
                        .then(data => ({
                            id: ind.id,
                            active: true,
                            name: ind.text,
